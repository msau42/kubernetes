...
`)
	...
	testCases := []struct {
	...
	}{
		{
			desc:     "success",
			want:   `
# HELP passwordfile_authentication_attempts_total Count of requests that authenticate with basic authentication based on status.
...
`,
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			if err := testutil.GatherAndCompare(prometheus.DefaultGatherer, strings.NewReader(tc.want), "passwordfile_authentication_attempts_total"); err != nil {
				t.Fatal(err)
			}
		})
	}
