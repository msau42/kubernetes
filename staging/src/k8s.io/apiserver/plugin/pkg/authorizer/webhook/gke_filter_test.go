...
func (d *DummyCongruentAuthorizer) Authorize(attr authorizer.Attributes) (decision authorizer.Decision, reason string, err error) {
...
func TestGkeApiserverAuthorize(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			decision, _, err := authorizer.Authorize(test.attr)
			if err != nil {
...
func TestNewGkeApiserverWebhookAuthorizer(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			authorizer, err := NewGkeApiserverWebhookAuthorizer(fmt.Sprintf("testdata/%s", test.file), time.Second, time.Second)
			if err != nil {
				t.Errorf("error occured: %v", err)
				return
			}
