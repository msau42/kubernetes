...
func NewGkeApiserverWebhookAuthorizer(kubeConfigFile string, version string, authorizedTTL, unauthorizedTTL time.Duration) (RuleResolverAuthorizer, error) {
	// Create regular WebhookAuthorizer
	webhookAuthorizer, err := New(kubeConfigFile, version, authorizedTTL, unauthorizedTTL)
	if err != nil {
		return nil, err
	}
